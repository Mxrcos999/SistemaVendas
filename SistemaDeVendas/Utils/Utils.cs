...
namespace Order
{
    public static class Utils
    {
        ...
        public static Customer RegisterCustomer()
        {
            ...
            try
            {
                var person = new Person(firstName, lastName, Convert.ToString(birthDate), cpf, email);
        ...
        public  static Product RegisterProduct()
        {
            ...
            var product = new Product(productDescription, Convert.ToString( productUnitaryValue),Convert.ToString( productAvailable));
            return product;
        }
        ...
        public static List<Customer> lista()
        {
            var custumers = new List<Customer>();
            var count = 1;
            foreach (var customer in custumers)
            {
                var teste = ($"{count} | {customer.Person?.FullName} | {customer.Person?.Cpf.CpfFormatado} | {customer.Person?.Email} | {customer.Person?.Age}");
                count++;
            }
            return custumers;
        }
        ...
        public static void ListCustomers(List<Customer> customers)
        {
            ...
            {
               // teste = ($"{count} | {customer.Person?.FullName} | {customer.Person?.Cpf.CpfFormatado} | {customer.Person?.Email} | {customer.Person?.Age}");
                Console.WriteLine();
                count++;
            }
        }
